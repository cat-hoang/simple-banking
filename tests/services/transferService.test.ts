...
import { applyTransfer, applyTransfers } from "../../src/services/transferService";
...
describe("applyTransfers", () => {
  ...
  function threeAccountBank(): Bank {
    return new Bank([new Account(A, 50000), new Account(B, 0), new Account(C, 20000)]);
  }
  ...
  it("should apply transfers cumulatively against the running balance", () => {
    ...
    applyTransfers(bank, [new Transfer(A, B, 30000), new Transfer(A, B, 30000)]);
